...
def export_to_excel(file_name, data_frame, cols):
    ...
    with pd.ExcelWriter(os.path.join("data/", f'{file_name}.xlsx')) as writer:
...
def report_save(data, data_type, data_created):
    data_created = data_created.strftime("%d-%m-%Y %H-%M")
    filename = f"report-{data_type}-{data_created}"

    if report_type == "json":
        with open(os.path.join("data/", f"{filename}.json"), "w") as report_json:
        ...
        df.to_csv(os.path.join("data/", fr"{filename}"), index=False, header=True)
...
def get_user_analises():
    ...
    tabcolor_status = config_user["tabcolor"].get("status")
    sequence_status = config_user["sequencies"].get("status")
    ...
    if sequence_alternate_status == "disable" and sequence_simple_status == "disable" \
            and sequence_basic_status == "disable" and tabcolor_status == "disable":
    ...
    if last_doubles:
        ...
        if sequence_status == "enable" and \
                (sequence.count("vermelho") >= sequence_limit or sequence.count("preto") >= sequence_limit):
        ...
        if not color and tabcolor_status == "enable":
            print('\rESTRATEGIA: TABELA DE CORES\r\n')

            if double[0][1] in [2, 3, 6, 8, 11, 12, 14]:
                color = "preto"
            elif double[0][1] in [1, 4, 5, 7, 10, 13]:
                color = "vermelho"
...
def start(demo, amount=2, stop_gain=None, stop_loss=None, martingale=None):
    ...
    count_loss = 0
    before_enter = None
    balance = float(get_balance()["balance"]) if not demo else float(100)
...
if __name__ == "__main__":
    ...
    # writer = pd.ExcelWriter(os.path.join("data/", f'{file_name}.xlsx'))
    #
